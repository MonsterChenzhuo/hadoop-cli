...
> Inventory is resolved from `$HADOOPCLI_INVENTORY`, `./cluster.yaml`, or `~/.hadoop-cli/cluster.yaml` unless `--inventory <path>` is passed. Pass `--inventory` explicitly when running against a non-default cluster or from an unrelated CWD.
